...
def loss_pde_residual(
    ...
    """Mean-squared Zerilli PDE residual (PINO term).

    r = u_tt - u_xx + V u on the interior, using 2nd-order central stencils.

    pred : (B, 1, Nt, Nx) or (B, Nt, Nx)
    V    : (B, Nx)         per-sample Zerilli potential
    """
    u = pred[:, 0] if pred.dim() == 4 else pred       # (B, Nt, Nx)
    u_tt = (u[:, 2:, :] - 2.0 * u[:, 1:-1, :] + u[:, :-2, :]) / (dt * dt)
    u_xx = (u[:, :, 2:] - 2.0 * u[:, :, 1:-1] + u[:, :, :-2]) / (dx * dx)
    u_tt = u_tt[:, :, 1:-1]                            # (B, Nt-2, Nx-2)
    u_xx = u_xx[:, 1:-1, :]
    u_int = u[:, 1:-1, 1:-1]
