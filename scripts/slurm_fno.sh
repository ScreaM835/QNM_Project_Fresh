#!/bin/bash
# ============================================================
# SLURM Job: FNO operator-learning pipeline
#   1) generate dataset (FD sweep)
#   2) train FNO
#   3) eval on test split (writes FD-schema .npz)
#   4) (optional) inverse-M demo
# ============================================================
...
# WORKDIR = directory you ran `sbatch` from (must be the project root).
...
echo ""
echo "[FNO] Training..."
python scripts/train_fno.py --config "$CONFIG"
...
# --- 4) Optional inverse demo (cheap, single FD call + 500 FNO steps) ---
echo ""
