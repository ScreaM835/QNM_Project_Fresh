...
# Greedy adaptive sampling (fraction=0.3, control experiment).
# ============================================================
#SBATCH --job-name=qnm_pinn_greedy_f03
...
CONFIG=configs/zerilli_l2_greedy_f03.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_greedy_f03/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_greedy_f03.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + Greedy sampling (f=0.3, control)..."
