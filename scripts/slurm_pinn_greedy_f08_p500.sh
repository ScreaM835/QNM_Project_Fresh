...
# Greedy adaptive sampling (fraction=0.8, period=500).
# ============================================================
#SBATCH --job-name=qnm_pinn_greedy_f08_p500
...
CONFIG=configs/zerilli_l2_greedy_f08_p500.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_greedy_f08_p500/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_greedy_f08_p500.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + Greedy sampling (f=0.8, p=500)..."
...
echo "  Greedy: period=500, greedy_fraction=0.8, candidates=100000"
