...
# Greedy f=0.3 + wider network [2,128,128,128,1].
# ============================================================
#SBATCH --job-name=qnm_pinn_greedy_f03_wide
...
CONFIG=configs/zerilli_l2_greedy_f03_wide.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_greedy_f03_wide/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_greedy_f03_wide.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training wider FNN + Greedy f=0.3..."
echo "  Model: FNN [2,128,128,128,1], tanh, Glorot uniform"
