...
# Greedy adaptive sampling (no exponential reweighting).
# ============================================================
#SBATCH --job-name=qnm_pinn_greedy
...
CONFIG=configs/zerilli_l2_greedy.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_greedy/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 — job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_greedy.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training FNN + Greedy sampling..."
...
echo "  Greedy: period=1000, greedy_fraction=0.5, candidates=100000"
