...
# Hard IC ansatz + Greedy f=0.3 + L-BFGS 30k.
# Mirrors slurm_pinn_greedy_f03_lbfgs30k.sh resources/workflow.
# ============================================================
#SBATCH --job-name=qnm_pinn_hard_ic_f03_lbfgs30k
...
CONFIG=configs/zerilli_l2_hard_ic_greedy_f03_lbfgs30k.yaml
CKPT_DIR="$WORKDIR/outputs/pinn/zerilli_l2_hard_ic_greedy_f03_lbfgs30k/checkpoints"

requeue_handler() {
    echo "[SIGNAL] Caught USR1 - job approaching time limit."
    echo "[SIGNAL] Resubmitting job to continue training..."
    sbatch "$WORKDIR/scripts/slurm_pinn_hard_ic_greedy_f03_lbfgs30k.sh"
    exit 0
}
...
echo ""
echo "============================================"
echo "[PINN] Training hard IC + Greedy f=0.3 + L-BFGS 30k..."
...
echo "  Hard IC: enabled via config flag"
