...
parameters are extracted from the Schwarzschild waveform at
$\xq=10\,M$ and from the Kerr waveform at $\scri$. The Schwarzschild
...
The Kerr extension uses the spin-$-2$ Teukolsky equation in
minimal-gauge horizon-penetrating hyperboloidal coordinates
...
\paragraph{Reference field and target mode.} The reference field is
...
The $N=201$ and $N=401$ fields are computed independently
of the $N=801$ fine reference field.
...
waveform at $\xq=10\,M$ and the Kerr waveform at $\scri$. Between the
...
\begin{table}[!htbp]
\centering
\caption{Schwarzschild QNM extraction windows at $\xq=10\,M$. The
reproduction uses only M1 and M2, following \citet{Patel2024pinn}.}
...
hFNO ($100$ configurations)
 & $[10,100]M$ & $t_0\in[10,25]M$, $\tend=100M$
 & $t_0\in[10,25]M$, $\tend\in[30,100]M$ \\
\bottomrule
\end{tabular*}
\end{table}
...
separate candidate generation, mode identification and robust
aggregation. The Leaver pole is used only for identification; it does
not replace any fitted value.
...
\paragraph{Candidate selection and consensus.} The Leaver damping
time $\tau_{\mathrm{ref}}$ sets all window scales. Targeted M3
...
$\omega_{R,\mathrm{ref}}$ are rejected. Frequency and damping-time
candidates are filtered independently using the larger of $12\%$ of
their median and three scaled median absolute deviations. The median
survivors are reported; their population standard deviation measures
inter-estimator spread rather than statistical uncertainty. Complete
per-estimator results are reported in Appendix~\ref{app:kerr-extractors}.
% --------------------------------------------------------------
\subsection{Reporting convention}
\label{sec:reporting}

Frequency and damping time are reported separately because phase
evolution primarily constrains $\omega$, whereas amplitude decay
constrains $\tau$ \citep{Berti2007ringdown,Patel2024pinn}.
...
multi-mode fits. We report both the pre-specified target-mode
ensemble and the retrospective Leaver-closest summary, with complete
extractor results in Appendix~\ref{app:kerr-extractors}. Each reported
