...
#SBATCH --output=qnm_kv2_%j.out
#SBATCH --error=qnm_kv2_%j.err
...
LOG=kerr/outputs/phase_b/kv2_${SLURM_JOB_ID}.log

echo "=== KV.2 Kerr self-convergence gate, job ${SLURM_JOB_ID} ===" | tee "${LOG}"
hostname | tee -a "${LOG}"
date     | tee -a "${LOG}"

"${PY}" -u kerr/scripts/kv2_convergence.py 2>&1 | tee -a "${LOG}"
