...
\begin{frame}{The PDE --- Zerilli equation}
  Linearising Einstein's equations on a Schwarzschild background and
  separating the angular variables yields the \textbf{1+1D wave equation}
  \[
    -\,\frac{\partial^{2}\Phi}{\partial t^{2}}
     \;+\; \frac{\partial^{2}\Phi}{\partial r_{\!*}^{\,2}}
     \;-\; V_{\ell}(r)\,\Phi \;=\; 0
  \]
  with the \textbf{Zerilli potential} (even parity, $\ell = 2$),
  \[
    V_{\ell}(r) \;=\;
      \Bigl(1 - \tfrac{2M}{r}\Bigr)
      \frac{2\bigl[\,n^{2}(n{+}1)r^{3}+3n^{2}Mr^{2}+9nM^{2}r+9M^{3}\bigr]}
           {r^{3}\,(nr+3M)^{2}},
    \quad n = \tfrac{1}{2}(\ell-1)(\ell+2),
  \]
  on the \textbf{tortoise coordinate} $\mathrm{d} r_{\!*} = \mathrm{d} r/(1-2M/r)$.

  \medskip
  \begin{itemize}
    \item Domain: $r_{\!*}/M \in [-50,\,150]$, $\;t/M \in [0,\,50]$.
    \item Boundary conditions: outgoing waves at $r_{\!*}\to+\infty$,
          ingoing at $r_{\!*}\to-\infty$ (horizon).
    \item Initial condition: localised Gaussian pulse at $t = 0$.
    \item \textbf{QNM frequency} (Leaver 1985):
          $M\omega = 0.3737 - 0.0890\,i$ $\;\Rightarrow\;$ $\tau/M = 11.241$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Forward PINN --- best result: waveform agreement}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \centering
      \includegraphics[height=0.70\textheight,keepaspectratio]{../outputs/pinn/zerilli_l2_greedy_f03_lbfgs30k/error_heatmap.png}
      \\[3pt]{\scriptsize Pointwise error $|\Phi_{\pinn{}}-\Phi_{\fd{}}|$ over the
      full $(x,t)$ domain.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[height=0.70\textheight,keepaspectratio]{../outputs/pinn/zerilli_l2_greedy_f03_lbfgs30k/ringdown_overlay.png}
...
% ══════════════════════════════════════════════════════════════════
\section{4. Extraction methods M3, M4, M5}
% ══════════════════════════════════════════════════════════════════
...
% ══════════════════════════════════════════════════════════════════
\section{5. Greedy adaptive resampling}
% ══════════════════════════════════════════════════════════════════
...
\begin{frame}{Greedy resampling --- error map: uniform vs.\ greedy $f\!=\!0.3$}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{../outputs/pinn/zerilli_l2_greedy_f03_lbfgs30k/error_heatmap.png}
...
\begin{frame}{Appendix --- M5 2-D heatmap, inverse PINN}
  \begin{center}
    \includegraphics[height=0.82\textheight,keepaspectratio]{../outputs/qnm/zerilli_l2_inverse_qnm_combo/pinn_method5_2d_heatmap.png}
